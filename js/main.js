...
// ===== 그만하기 모달 =====
function openQuitModal() { quitModal.classList.add('active'); }
function closeQuitModal() { quitModal.classList.remove('active'); }

quitBtn.addEventListener('click', openQuitModal);
modalCancelBtn.addEventListener('click', closeQuitModal);
modalConfirmBtn.addEventListener('click', () => { closeQuitModal(); goHome(); });

quitModal.addEventListener('click', (e) => { if (e.target === quitModal) closeQuitModal(); });
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && quitModal.classList.contains('active')) closeQuitModal();
});
...
function startQuiz() {
  ...
  if (pool.length === 0) { alert('선택한 카테고리에 문제가 없어요!'); return; }
...
function renderQuestion() {
  ...
  document.getElementById('feedbackBox').classList.remove('visible', 'feedback-wrong');
  document.querySelector('.question-area').style.animation = 'none';
  setTimeout(() => { document.querySelector('.question-area').style.animation = 'fadeIn 0.4s ease'; }, 10);
}
...
function showResult() {
  ...
  document.getElementById('wrongCount').textContent = total - state.correctCount;
...
function getGrade(rate) {
  if (rate === 100) return { emoji:'🏆', title:'완벽해요!', grade:'⭐⭐⭐ 환경 박사!', color:'#f57c00', message:'와! 100점 만점이에요! 당신은 진짜 환경 전문가예요! 지구가 당신 덕분에 웃고 있어요. 🌍💚' };
  if (rate >= 90)  return { emoji:'🥇', title:'훌륭해요!', grade:'⭐⭐⭐ 환경 지킴이!', color:'#e65100', message:'거의 완벽해요! 환경에 대해 정말 잘 알고 있군요. 조금만 더 공부하면 만점도 금방이에요!' };
  if (rate >= 70)  return { emoji:'🥈', title:'잘했어요!', grade:'⭐⭐ 환경 탐험가!', color:'#1565c0', message:'아주 잘했어요! 환경에 대한 지식이 쑥쑥 자라고 있어요. 틀린 문제를 다시 읽어보면 더 잘할 수 있어요!' };
  if (rate >= 50)  return { emoji:'🥉', title:'괜찮아요!', grade:'⭐ 환경 새싹!', color:'#2e7d32', message:'절반 이상 맞혔어요! 환경 공부를 더 하면 점점 좋아질 거예요. 다시 도전해 보세요! 🌱' };
  return { emoji:'🌱', title:'도전하세요!', grade:'환경 씨앗!', color:'#6a1b9a', message:'걱정하지 마세요! 틀린 문제를 보며 하나씩 배워가면 돼요. 다시 도전하면 분명 더 잘할 수 있어요! 💪' };
}
